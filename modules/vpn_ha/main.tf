...
resource "google_compute_subnetwork" "main_subnet" {
  count         = var.create_network ? 1 : 0  
  name          = "${var.network_name}-subnet-main"
  project       = var.project_id
  region        = var.region
...
resource "google_compute_external_vpn_gateway" "external_gateway" {
  ...
  name            = var.peer_external_gateway.name != null ? var.peer_external_gateway.name : "external-${var.network_name}"
  project         = var.project_id
...
resource "google_compute_router_peer" "bgp_peer" {
  ...
  region          = var.region
  project         = var.project_id
  name            = each.value.bgp_session_name != null ? each.value.bgp_session_name : "${var.network_name}-${each.key}"
  router          = local.router
...
locals {
  host_peerings = {
    for env, cfg in var.host_networks :
    env => "projects/${cfg.project_id}/global/networks/${cfg.network_name}"
  }
}

resource "google_compute_network_peering" "vpn_to_hosts" {
  for_each = local.host_peerings

  name         = "peering-vpn-to-host-${each.key}"
  network      = google_compute_network.vpc[0].self_link
